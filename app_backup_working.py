...
if menu == "Strategic Performance Metrics":
    ...
    if df_herd.empty:
        ...
        small_female_count = len(
            active_animals[active_animals["category"] == "Small Sheep - Female"]
        )
        m5.metric("Small - Female", small_female_count)

        # 5. Small Sheep - Male
        small_male_count = len(
            active_animals[active_animals["category"] == "Small Sheep - Male"]
        )
...
# 📝 MODULE 2: ACTIVE HERD REGISTRY
    ...
    with tab2:
        ...
        if df_herd.empty:
            ...
            active_list = df_herd[df_herd["status"] == "Active/Healthy"][
                "tag_no"
            ].tolist()
            if not active_list:
                st.info(
...
# 🌾 MODULE 5: FEED INVENTORY & RECIPE CALCULATOR
    ...
    # 🌟 STRUCTURAL CHECKS
    # =================New section copied from Gemini===========================
    # Convert cloud data list to a DataFrame for compatibility with your layout structures
    if (
        "cached_recipes" in st.session_state
        and st.session_state.cached_recipes is not None
    ):
        df_recipes = pd.DataFrame(st.session_state.cached_recipes)
    else:
        df_recipes = pd.DataFrame(
            columns=["recipe_type", "calculated_mix_cost_per_kg", "recipe_breakdown"]
        )

    # Store back into your active session tracker
    ...
    with tab1:
        ...
        if st.button(
            ...
            if total_fattening != 100:
                ...
                try:
                    # Connect to your master cloud database
                    conn = db.create_connection()
                    cursor = conn.cursor()

                    # PostgreSQL cloud command using %s markers and ON CONFLICT
                    cursor.execute(
                        """
                    ...
                    st.error(f"Fattening Cloud Connection Failed: {e}")
                    st.stop()
        ...
        if st.button(
            ...
            if total_general != 100:
                ...
                try:
                    ...
                    st.error(f"General Herd Cloud Connection Failed: {e}")
                    st.stop()
    ...
    # === SAFE DATA PREPARATION LAYER (LOCAL ONLY FOR NOW) ===
    # We use the df_inv loaded on line 762 from your local database file
    ...
    if isinstance(df_inv, pd.DataFrame) and not df_inv.empty:
        if "is_active" in df_inv.columns:
            # Safely handle both integer (1/0) and boolean values from your local rows
        ...
        # This extracts your local items: Corn, Wheat, Radda, Bean Hay
        ...
        with col_deact:
            ...
            with st.form(key="deactivate_form_isolated"):
                if active_item_options:
                    ...
                    # FIX: Renders a harmless, disabled button to fulfill Streamlit's structural rule
        ...
        with col_react:
            ...
            with st.form(key="reactivate_form_isolated"):
                ...
                if inactive_options:
                    ...
                    # FIX: Keeps Streamlit happy when no inactive items exist
...
# ===========================================================================
# This module provides a direct interface for users to edit existing records in the SQLite database tables through an interactive spreadsheet-like data editor. Users can select which table they want to edit, make changes directly in the displayed grid, and then save those changes back to the database. This allows for quick corrections or updates without needing to write SQL commands or use external database management tools.
# However, it also comes with a warning that changes made here will permanently modify the database records, so it should be used with caution.
# The implementation uses Streamlit's `st.data_editor` to create an editable grid for the selected table, and when the user clicks the save button,
# it overwrites the existing table in the database with the modified DataFrame.
# This module is particularly useful for making quick fixes to data entry errors or updating records without needing to go through the
# form interfaces provided in the other modules.
# Note: The `st.data_editor` component allows for dynamic editing of the DataFrame, and the changes are only saved to the database when the user clicks the save button.
# This provides a layer of control over when changes are committed to the database.
# The code also includes error handling to catch any issues that may arise during the database update process, ensuring that users are informed if something goes wrong.
# Overall, this module enhances the usability of the application by providing a straightforward way to manage and correct data directly within the app interface.
# ==========================================================================
